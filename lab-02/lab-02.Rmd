...
all_bellevue <- todo_1 %>%
  summarize(
    mv = mean(total_val, na.rm = TRUE),
    vv = var(total_val, na.rm = TRUE)
  )
...
mean_diff <- all_bellevue$mv - lake_views$mv
var_diff <- all_bellevue$vv - lake_views$vv
```

The difference between the average total value for all properties in Bellevue and all properties with the view to both Lake Washington and Sammammish in Bellevue is `r mean_diff`, and the difference in variance is `r var_diff`.
...
kingco_homes_clean %>%
  ...
  group_by(condition) %>%
  summarise(
    avg_val_per_sf = mean((land_val + imp_val)/sqft, na.rm = TRUE),
    count = n()
  )
```
...
The histogram with the larger sample has a narrower distribution that stays closer to the mean, while the one with the smaller sample is wider and more spread out. As sample size increases, the sample means vary less and the distribution becomes more normal, which is exactly what the Central Limit Theorem describes.
...
# TODO
task_3 <- kingco_sales %>%
  select(city, sale_price, beds) %>%
  filter(
    city == "SEATTLE",
    beds > 0 # was getting Inf since there were 0 values
    ) %>%
  mutate(price_per_bed = sale_price/beds)
task_3

task_3 %>%
  arrange(desc(price_per_bed)) %>%
  slice_max(order_by = price_per_bed, n = 5)

task_3 %>%  
  summarise(
    avg_ppb = mean(price_per_bed, na.rm = TRUE),
    max_ppb = max(price_per_bed, na.rm = TRUE),
    min_ppb = min(price_per_bed, na.rm = TRUE),
    median_ppb = median(price_per_bed, na.rm = TRUE)
  )
```
The average price per bedroom in Seattle is around $216,000, and the median is $175,000. When the average is so much greater than the median, that indicates that there are a few very expensive properties that are skewing the average to the right/upward. The price per bedroom metric has a wide range in Seattle and the top 5 properties are all high priced homes with only a few bedrooms, so that also inflates the metric. Overall, most homes fall in a moderate range, and there are a small number of significant luxury outliers.
