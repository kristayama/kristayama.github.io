...
Theme: Exploring how luxury and convenience features (granite countertops, gyms, pools, etc.) in rental listings are associated with higher advertised rent prices
Write Up:
For this analysis, I explored how various apartment features influence rent prices using several visualizations. The jittered boxplots for GYM and GRANITE showed that some features, like gyms, had little apparent effect on rent, while the granite plot suggested that apartments without granite sometimes had higher rents, though the difference was not substantial. The scatter plot of the total number of luxury features (LUXURY_COUNT) versus rent also revealed no strong trend; high rents were observed across all feature counts.

The heatmap of average rent by feature presence was particularly informative, clearly highlighting that listings with DOORMAN, FURNISHED, GARAGE, GRANITE, and STAINLESS had noticeably higher average rents compared to those without these features. This observation was confirmed and quantified in the bar plot of average rent differences, which showed that GARAGE and FURNISHED contributed the largest increases in average rent among the selected features. Overall, these visualizations illustrate that while some amenities have little effect, a few key features meaningfully increase rent, and the combination of heatmaps and difference plots makes these relationships easy to see.

Plot visualization and interpretation with help from ChatGPT
Citation:
OpenAI, Response to: "Help with creative ggplot2 visualizations for rental dataset assignment", ChatGPT, (Oct. 29, 2025), https://chat.openai.com/
 (enter query into "Message ChatGPT" box).
```{r plots!}
...
```
Reflection:
For the heatmap, I stretched the color scale so that even small differences in average rent between listings with and without certain features appeared visually large. This exaggerates the perception of impact while keeping the underlying data accurate. The bright contrast draws attention to minor differences, making them seem more important than they actually are.
This shows how subtle choices — axis scaling, color intensity, and bar size — can influence interpretation. Even without altering the actual numbers, viewers can easily be misled, highlighting the ethical responsibility of designers to present data honestly.

With help from ChatGPT
Citation:
OpenAI, Response to: "How can I create a subtly misleading heatmap for rent features in R?", ChatGPT, (Oct. 29, 2025, 8:35 PM), https://chat.openai.com/
 (enter query into "Message ChatGPT" box).
